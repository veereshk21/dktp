...
const ComboOrder = (props) => { // eslint-disable-line
  return (
    <section>
      {(props.comboOrder) ?
        <div>
          {props.subOrders && props.subOrders.length > 0 && <div>
            {props.subOrders.map((order) => (
              <div className="margin10 noSideMargin" key={order.clientReferenceNumber}>
                <h3>
                  <span>{props.cqKeys.label['DT_OD_CONFIRMATION_DEVICE_' + order.suborderType + '_TEXT']}</span>
          ...
          {props.multiOrderDetails && props.multiOrderDetails.length > 0 && <div>
            {props.multiOrderDetails.map((order) => (
              <div className="margin10 noSideMargin" key={order.clientOrderRefernceNumber}>
                { order.flow === 'EUP' ? <h3>
                  <span>{props.cqKeys.label.DT_OD_CONFIRMATION_DEVICE_UPGRADE_TEXT}</span>
                  <span className="pad5">{order.clientOrderRefernceNumber}</span>
                </h3> : <h3>
                  <span>{props.cqKeys.label.DT_OD_CONFIRMATION_DEVICE_ADD_TEXT}</span>
                  <span className="pad5">{order.clientOrderRefernceNumber}</span>
                </h3>}
              </div>
            ))}
            <p className="margin5 noSideMargin">{props.cqKeys.label.DT_OD_CONFIRMATION_DEVICE_PROCESS_TEXT_1}{props.multiOrderDetails.length}{props.cqKeys.label.DT_OD_CONFIRMATION_DEVICE_PROCESS_TEXT_2}</p>
          </div>}
        </div> }
    </section>
  );
